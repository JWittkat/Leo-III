...
object AxiomsForArithmetic {
  final def apply()(implicit state: State[AnnotatedClause], sigArithmetic: SignatureArithmetic): Unit = {
    ...
    // val containsProduct = sig.exists("$$product")
    // if it only contains sums, than add the sum stuff
    if (sigArithmetic.containsAdd() || sigArithmetic.containsOrd()) {
      println("ADD AXIOMS FOR ADDITION")
    ...
    // if it contains less than add axioms for sum and orderings
    if (sigArithmetic.containsOrd()) {
      println("ADD AXIOMS FOR ORDERING")
      if (sigArithmetic.containsInt()) {
    ...
    if (sigArithmetic.containsMult()) {
      println("ADD AXIOMS FOR MULTIPLICATION")
      if(sigArithmetic.containsInt()) {
    ...
    // check in which situation which one to apply
    // axioms += getAxiomsOrdering(int)
